...
function App() {
  ...
  const handleDataLoaded = useCallback((newData: RNCRecord[]) => {
    ...
    setRncData(prev => {
      // 1. Create a Map of existing items by ID (for O(1) lookups)
      //    This ensures we respect the stable ID (RNC-{digits})
      const dataMap = new Map<string, RNCRecord>();
      ...
      // 2. Process imports: UPSERT (Update existing, Insert new)
      validImports.forEach(importedItem => {
        // Using the ID generated by the parser (RNC-{digits})
        if (dataMap.has(importedItem.id)) {
          // UPDATE: Replace existing record with new one
          dataMap.set(importedItem.id, importedItem);
          updatedCount++;
        } else {
      ...
      if (updatedCount > 0 || newCount > 0) {
        toast.success(`Importação concluída: ${updatedCount} atualizadas, ${newCount} novas`);
      } else {
