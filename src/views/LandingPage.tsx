...
export const LandingPage: React.FC<LandingPageProps> = ({ onDataLoaded, onEnterDashboard, savedDataCount = 0 }) => {
  const [loading, setLoading] = useState(false);
  const [totalLoaded, setTotalLoaded] = useState(savedDataCount);
  const [processMessage, setProcessMessage] = useState('');

  useEffect(() => {
    // Force check localStorage on mount
    const saved = localStorage.getItem('savedRNCs');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          setTotalLoaded(parsed.length);
          // If we have saved data count from props, prefer that, but if prop is 0 and storage has data, Update.
          // Actually, if we are here, it means we found data.
          if (parsed.length > 0) {
            // onDataLoaded(parsed); // Optional: we might not want to re-inject upstream if App already has it? 
            // But the prompt says "Acessar Dados Processados" remains disabled. 
            // Enabling it locally is enough, App will redirect or use its own state when clicked.
          }
        }
      } catch (e) {
        console.error("Error checking local storage in LandingPage", e);
      }
    }
  }, []);
    ...
    <div className="relative flex min-h-screen w-full flex-col items-center overflow-hidden bg-background-dark font-display selection:bg-primary selection:text-black">
      ...
      <main className="relative z-10 flex w-full grow flex-col justify-center px-4 py-8">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          {processMessage && (
            <div className={`mt-4 rounded-full px-6 py-2 text-sm font-bold border animate-fade-in ${processMessage.includes('100%')
        ...
        <div className="mt-12 flex w-full flex-col items-center gap-6">
          ...
          <button
            ...
            className={`w-full max-w-md group text-left transition-all duration-300 ${totalLoaded > 0 ? 'cursor-pointer opacity-100' : 'cursor-not-allowed opacity-50 grayscale'
              }`}
          >
          ...
          <div className="mt-6 w-full text-center">
            <div className="inline-block rounded-full border border-primary/50 px-8 py-2 text-primary shadow-[0_0_12px_rgba(0,212,106,0.3)] bg-primary/10 backdrop-blur-md text-sm font-semibold">
              {totalLoaded > 0
                ? `${totalLoaded} registros recuperados da memória.`
